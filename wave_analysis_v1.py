...
for checking_folder in subfolders:
    for checking_loc in model_locs:
        ...
        matching_files = list(Path(dir_modeled_data + modeled_data_name).glob(f'Buoy*'))
        modeled_data = pd.DataFrame(columns=modeled_all_vars)

        for loading_file in matching_files:
            clean_lines = []
        ...
        for param_checking in wave_params[1:]:
            ...
            if '파주기' in param_checking:
                fname_save = f'{checking_loc}_{param_checking}_{wave_period_var}'
            else: 
                fname_save = f'{checking_loc}_{param_checking}'
            fig_title = f'{checking_loc} ({model_locs[checking_loc]}) {df_wave_combine.iloc[0,0].date()} - {df_wave_combine.iloc[-1,0].date()}'
